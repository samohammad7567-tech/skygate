...
/// `showDatePicker` asserts its initial date sits inside the range it is
/// given. "حدد مدة الرحلة" hands it a `firstDate` of tomorrow — the API wants
/// `preferred_start_date` after today — so a field that still opened on
/// today's date threw the moment it was tapped.
void main() {
  setUpAll(() async {
  ...
  testWidgets('a field whose range starts tomorrow opens without throwing', (
    ...
    await pump(
      ...
      Scaffold(
        body: AppDateField(
          hint: 'start_date',
          // Nothing picked yet, so the field would fall back to today — the
          // one date this range excludes.
          value: null,
          firstDate: tomorrow,
          onPicked: (_) {},
        ),
  ...
  testWidgets('both dates on the VIP duration step open', (tester) async {
    ...
    for (final index in [0, 1]) {
      ...
      // Dismissed by popping the route rather than by tapping "إلغاء": the
      // button's label is whatever MaterialLocalizations gives it, which is
