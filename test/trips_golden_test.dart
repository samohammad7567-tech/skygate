...
/// "رحلاتي" against the shape `app/my-trips` actually answers with: trips, not
/// bookings, each carrying the pilgrim's own `booking` block, the bucket the
/// API put it in, its length, and the modes it is travelled by.
///
/// The first trip is the one the design draws — five legs with the second
/// under way. The second is the demo payload verbatim, legs and photo absent,
/// which is what most trips come back as today.
...
void main() {
  ...
  group('the my-trips payload', () {
    ...
    test('prefers the API\'s own duration over the two dates', () {
      // 03-03 to 03-10 counts eight days inclusively; the API says seven, and
      // the API wins.
