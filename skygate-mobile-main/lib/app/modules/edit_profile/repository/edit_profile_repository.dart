...
import 'package:sky_gate/app/modules/edit_profile/params/edit_profile_params.dart';
...
class EditProfileRepository {
  ...
  Future<Either<Failure, BaseResponse<UserModel>>> editUserProfile({required EditProfileParams params, required File? avatar_image}) async {
    try {
      ...
      if (params.national_id_images!.isNotEmpty) {
        params.national_id_images!.forEach((element) async {
          request.files.add(http.MultipartFile.fromBytes(
              'national_id_images', await element.readAsBytes(),
      ...
      if (params.passport_images!.isNotEmpty) {
        params.passport_images!.forEach((element) async {
          request.files.add(http.MultipartFile.fromBytes(
              'passport_images', await element.readAsBytes(),
      ...
      final result = BaseResponse.fromJson(resultEncoded, (json) => UserModel.fromJSON(json));
