...
import 'package:sky_gate/app/modules/change-operation/bindings/change_operation_binding.dart';
import 'package:sky_gate/app/modules/change-operation/models/change_request_model.dart';
import 'package:sky_gate/app/modules/change-operation/repositories/get_change_request_repository.dart';
import 'package:sky_gate/app/modules/change-operation/repositories/get_regular_trips_repository.dart';
import 'package:sky_gate/app/modules/change-operation/repositories/pay_change_request_repository.dart';
import 'package:sky_gate/app/modules/change-operation/repositories/submit_change_request_repository.dart';
import 'package:sky_gate/app/modules/change-operation/views/after_payment_method_view.dart';
import 'package:sky_gate/app/modules/change-operation/views/change_operation_last_step_view.dart';
import 'package:sky_gate/app/modules/change-operation/views/change_request_result_view.dart';
import 'package:sky_gate/app/modules/change-operation/views/ticket_edit_success_view.dart';
...
import 'package:sky_gate/app/modules/my-bookings-requests/repository/get_payment_methods_repository.dart';
...
import 'package:sky_gate/app/modules/refund-operation/repository/accept_booking_request_conditions_repository.dart';
import 'package:sky_gate/app/modules/refund-operation/repository/get_booking_request_repository.dart';
import 'package:sky_gate/app/modules/user/repository/get_user_repository.dart';
...
class ChangeOperationController extends GetxController
  ...
  void detectNavigationPath() {
    if (changeRequest.status == "1") {
      Get.off(() => ChangeOperationLastStepView(),
          binding: ChangeOperationBinding(), arguments: {"id": bookingID});
    } else if (changeRequest.status == "2") {
      if ((changeRequest.payment_method != null &&
          changeRequest.payment_method != "")) {
        Get.off(() => AfterPaymentMethodView(),
            binding: ChangeOperationBinding(), arguments: {"id": bookingID});
      } else {
        Get.off(() => ChangeRequestResultView(),
            binding: ChangeOperationBinding(), arguments: {"id": bookingID});
      }
    } else if (changeRequest.status == "3") {
      Get.off(() => TicketEditSuccessView(),
          binding: ChangeOperationBinding(), arguments: {"id": bookingID});
    } else {
      return;
    }
    ...
    (await getChangeRequestRepository.getChangeRequest(
      ...
      if (right.code == "1") {
        ...
        if (changeRequest.plane_missing_penalty!.contains(".")) {
          penaltyTotalSum = double.parse(changeRequest.plane_missing_penalty!) +
