...
class GetTravelAllowedCountriesRepository {
  ...
  Future<Either<Failure, BaseResponse<List<TravelAllowedCountryModel>>>>
  getTravelAllowedCountries() async {
    try {
      return right(
        await httpHelper.get(
          ...
          decoder: (json) {
            if (json != null) {
              return json
                  .map<TravelAllowedCountryModel>(
                    (json) => TravelAllowedCountryModel.fromJson(json),
                  )
                  .toList();
            } else {
              return [];
