...
class AirportTaxiRequestRepository {
  ...
  Future<Either<Failure, BaseResponse<bool>>> sendTaxiRequest({
    AirportTaxiParams? params,
  }) async {
  ...
  Future<Either<Failure, BaseResponse<List<AirportTaxiRequestModel>>>>
  getRequests() async {
    Map<String, dynamic> requestBody = {};
      ...
      return right(
        await httpHelper.post(
          ...
          decoder: (json) {
            if (json != null) {
              return json
                  .map<AirportTaxiRequestModel>(
                    (json) => AirportTaxiRequestModel.fromJson(json),
                  )
                  .toList();
            } else {
              return [];
