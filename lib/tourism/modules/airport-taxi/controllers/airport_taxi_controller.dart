...
import 'package:skygate/tourism/modules/airport-taxi/repository/airport_taxi_request_repository.dart';
...
class AirportTaxiController extends GetxController {
  ...
  GetAirportTaxiRequestsDataStatus getAirportTaxiRequestsDataStatus =
  ...
  List<String> directions = ["من المنزل إلى المطار", "من المطار إلى المنزل"];
  ...
  Future<void> sendTaxiRequest({BuildContext? context}) async {
    ...
    (await airportTaxiRequestRepository.sendTaxiRequest(params: params)).fold(
      (left) {
  ...
  Future<void> getAirportTaxiRequests() async {
    ...
    (await airportTaxiRequestRepository.getRequests()).fold(
      (left) {
      ...
      (right) async {
        if (right.code == "1") {
          ...
          getAirportTaxiRequestsDataStatus =
              GetAirportTaxiRequestsDataStatus.success;
          update();
        } else {
          getAirportTaxiRequestsDataFailure = CustomFailure(
  ...
  Future<void> getAirportsData() async {
    (await getAirportsRepository.getAirports()).fold((left) {}, (right) async {
      if (right.code == "1") {
        airports = right.data!;
      } else {
        airports = [];
