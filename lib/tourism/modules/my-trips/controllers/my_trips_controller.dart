...
import 'package:skygate/tourism/modules/my-trips/models/trip_model.dart';
import 'package:skygate/tourism/modules/my-trips/repository/accept_trip_conditions_repository.dart';
import 'package:skygate/tourism/modules/my-trips/repository/add_trip_notes_repository.dart';
import 'package:skygate/tourism/modules/my-trips/repository/get_trips_repository.dart';
...
class MyTripsController extends GetxController {
  ...
  AcceptTripConditionsStatus acceptTripConditionsStatus =
      AcceptTripConditionsStatus.initial;
  ...
  Future<void> getTripsData() async {
    ...
    (await getTripsRepository.getTrips()).fold(
      (left) {
      ...
      (right) async {
        if (right.code == "1") {
          ...
          toastification.show(
            ...
            title: const Text("طلب تعديل/إلغاء الرحلة"),
            description: Text(
              "لقد تم إرسال طلبك بنجاح. سيتم الرد في أقرب وقت.",
            ),
  ...
  Future<void> acceptTripConditions({BuildContext? context}) async {
    ...
    (await acceptTripConditionsRepository.acceptTripConditions(
      ticketID: ticketID,
    )).fold(
      (left) {
  ...
  Future<void> addTripNotes({BuildContext? context}) async {
    ...
    (await addTripNotesRepository.addTripNotes(
      ticketID: ticketID,
      notes: notes,
    )).fold(
      (left) {
