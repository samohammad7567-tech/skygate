...
class SalesSupportController extends GetxController with WidgetsBindingObserver {
  ...
  Future<void> readMessages() async {
    ...
    (await messagesRepository.getMessages(targetTeam: AppConfig.salesTeam.toString()))
        .fold((left) {
      ...
      if(right.code == "1") {
        right.data!.forEach((element) {
          if(element.sender_type==AppConfig.appUserType) {
            messages.add(
                ChatMessage(text: element.message!,
                    isMe: true,
