...
import 'package:skygate/tourism/modules/messages/models/chat_message_model.dart';
...
class CustomerCareSupportController extends GetxController with WidgetsBindingObserver {
  ...
  Future<void> readMessages() async {
    ...
    (await messagesRepository.getMessages(targetTeam: AppConfig.customerCareTeam.toString()))
        .fold((left) {
            ...
            if(element.sender_type==AppConfig.adminUserType) {
              messages.add(
                  ChatMessage(
                      text: element.message!,
                      isMe: false,
                      timestamp: DateTime.parse(element.updated_at!),
  ...
  Future<void> sendUserMessage({BuildContext? context}) async {
    ...
    (await messagesRepository.sendMessage(
      ...
      toastification.show(
        context: context,
        title: const Text("محادثة رعاية الزبائن"),
