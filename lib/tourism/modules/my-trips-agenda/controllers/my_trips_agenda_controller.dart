...
import 'package:skygate/tourism/modules/airport-taxi/repository/get_airports_repository.dart';
...
import 'package:skygate/tourism/modules/my-trips-agenda/params/submit_booking_request_params.dart';
import 'package:skygate/tourism/modules/my-trips-agenda/repository/get_cities_repository.dart';
import 'package:skygate/tourism/modules/my-trips-agenda/repository/search_regular_trips_repository.dart';
import 'package:skygate/tourism/modules/my-trips-agenda/repository/submit_booking_request_repository.dart';
...
class MyTripsAgendaController extends GetxController {
  ...
  TextEditingController babiesCountController = TextEditingController(
    text: "0",
  );
  TextEditingController adultsCountController = TextEditingController(
    text: "0",
  );
  ...
  void onInit() async {
    ...
    for (var element in AppConfig.arabicNationalities) {
    ...
    for (var element in cities) {
      departureCitiesDropdownItems.add(
        DropdownMenuItem(
          value: "${element.city}",
      ...
      arrivalCitiesDropdownItems.add(
        DropdownMenuItem(
          value: "${element.city}",
          child: Text("${element.city}"),
        ),
  ...
  void isTripWithinDate(bool selectReturnTripPage) {
    if (selectReturnTripPage == true) {
      ...
      for (var element in secondWayTrips) {
      ...
      for (var element in firstWayTrips) {
        final startDate = DateTime.tryParse(element.repeat_start_date!);
        final endDate = DateTime.tryParse(element.repeat_end_date!);
        final isInDateRange =
            currentDate.isAfter(startDate!.subtract(const Duration(days: 1))) &&
  ...
  TripModel getTripData({bool? isFirstWay}) {
    if (isFirstWay == true) {
      final index = firstWayTrips.indexWhere(
        (item) => item.id == selectedFirstWayTripID,
      );
      ...
      if (isOneWay == true) {
        ...
        final index = secondWayTrips.indexWhere(
          (item) => item.id == selectedSecondWayTripID,
        );
  ...
  Future<void> getAirportsData() async {
    ...
    update();
    (await getAirportsRepository.getAirports()).fold(
      (left) {
  ...
  Future<void> getCitiesData() async {
    ...
    update();
    (await getCitiesRepository.getCities()).fold(
      (left) {
        getAirportsDataFailure = left;
  ...
  Future<void> submitBookingRequest({
    String? isRegularTrip,
    String? regularTripID,
  }) async {
    ...
    (await submitBookingRequestRepository.submit(params: params)).fold(
      (left) {
