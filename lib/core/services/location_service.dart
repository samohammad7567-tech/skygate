...
// Imported for `openAppSettings` alone. It ships its own `PermissionStatus`,
// which would collide with the `location` one this file switches on, so the
// whole package is kept behind a prefix.
...
class LocationService {
  ...
  static Future<LocationAccess> request() async {
    ...
    // `deniedForever` raises no dialog, so asking again would hang the button
    // on a prompt that never appears.
  ...
  static LocationAccess _map(PermissionStatus status) => switch (status) {
    // `grantedLimited` is iOS 14's "reduced accuracy". It still yields a fix,
    // just a coarser one, which the map would rather have than nothing.
