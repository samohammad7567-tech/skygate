...
class JourneyAssets {
  ...
  /// Extra covers cycled through when a hotel comes back from the API with no
  /// photo of its own. Generate them as a set — one hotel chain's house style,
  /// not three unrelated stock shots — and keep people out of frame.
  ///
  /// Spec: 1536x1024 source, exported at 768x512, JPEG q82. These are
  /// photographic and full-bleed, where JPEG saves roughly 8x over PNG.
  ///
  /// Deliberately absent from [all] until the files land; the bundle smoke test
  /// asserts every path in that list resolves.
  ...
  /// The covers above in cycle order, starting with the one that already
  /// ships. Index into it by hotel id so a given hotel keeps the same cover
  /// between rebuilds.
  ...
  /// Every distinct shipped file above, for bundle smoke tests. The aliases
  /// ([activities], [clockFrom]) are covered by what they point at, and the
  /// generated hotel covers are held out until their files exist.
