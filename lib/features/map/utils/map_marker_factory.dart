...
class MapMarkerFactory {
  ...
  static Future<BitmapDescriptor> activityPin({
    ...
    final pin = await _paint(
      color: color,
      // The glyph sits inside the white plate with a rim of it showing.
      contentScale: 0.72,
  ...
  static Future<BitmapDescriptor> pilgrimPin({
    ...
    final pin = await _paint(
      color: color,
      // The photo fills the white plate rather than sitting on it, so the
      // plate reads as a ring around the face exactly as in the design.
      contentScale: 1,
      // A photo is content in its own right, not a glyph to be re-tinted.
      tintContent: false,
      content: (canvas, center, radius) {
  ...
  static Future<BitmapDescriptor> _paint({
    ...
    // A glyph is re-tinted in the drop's own colour, so one exported in some
    // other palette still comes out on-brand; a photo is drawn as it is.
    if (tintContent) {
      canvas.saveLayer(
  ...
  static Path _dropPath({required double radius, required double height}) {
    final center = Offset(radius, radius);
    // Where the tail leaves the disc — far enough round it that the two meet
    // as one shape rather than as a circle with a spike stuck on.
