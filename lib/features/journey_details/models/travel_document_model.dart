...
/// Where a visa application has got to. The four standings the design draws,
/// read leniently because the API types `status` as undocumented strings.
enum VisaStatus {
...
/// A visa on a pilgrim's file. `app/visas` publishes no schema, so the fields
/// are read through the spellings the neighbouring resources use.
class VisaModel {
  int? id;
...
/// A ticket issued for one pilgrim on one leg. `trip_segment_id` is what ties
/// it to the segment whose tab is showing.
