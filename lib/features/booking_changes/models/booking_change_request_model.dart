...
/// Where an amendment request stands with the office.
///
/// The API answers with whatever wording the back office configured, so the
/// standing is read out of the label rather than matched against a fixed set
/// of codes — the same way [BookingStatus] and [PrivateTripStatus] read theirs.
...
/// What the pilgrim asked to change. The five codes are the ones the API
/// accepts; anything else keeps the label the API sent, so a type added on the
/// server still reads correctly here instead of collapsing into "other".
...
class BookingChangeRequestModel {
  ...
  /// The type exactly as the API worded it. Printed in place of the
  /// translation whenever the code is one this build does not know.
  String? typeLabel;
  ...
  // ── Read opportunistically off an embedded booking ───────────────────────
  // The published resource carries only `booking_id`; a response that nests
  // the booking lets the card name the trip the request belongs to.
  String? tripTitle;
  ...
  /// The booking this request hangs off, named the way the card prints it:
  /// its own reference when the response carries one, its id otherwise.
