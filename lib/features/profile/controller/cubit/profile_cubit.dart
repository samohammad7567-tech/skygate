...
class ProfileCubit extends Cubit<ProfileState> {
  ...
  Future<void> savePassport() async {
    ...
    emit(PassportSaving());
    try {
      // `toPilgrimJson` builds the body `POST app/pilgrims` takes, and the PUT
      // example is a subset of it, so one builder serves both. `is_self` is
      // true: "حسابي" only ever edits the account holder's own passport.
      await DioService.put(
        ApiEndpoints.pilgrim(id),
